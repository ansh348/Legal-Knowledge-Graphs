...
\paragraph{ECHR cross-jurisdiction set.}
For cross-jurisdictional evaluation we use 297 full-judgment ECHR merits decisions scraped from HUDOC (Turkey-respondent cases) \cite{chalkidis2019neural}: 262 violation, 35 no-violation (88\% imbalance). A separately compiled 55-concept ECHR ontology covers Convention articles, doctrinal tests (margin of appreciation, proportionality, prescribed-by-law), and procedural principles (exhaustion of domestic remedies, victim status, four-month rule). Extraction is performed by GPT-4.1 mini \cite{openai2025gpt41} rather than a frontier model, deliberately testing pipeline robustness under weaker extraction. The blinding protocol is identical to IL-TUR (outcome-bearing graph fields withheld; regex-based outcome removal from raw text, plus the HUDOC metadata banner stripped), and the raw-text character budget is set to $\approx$1.05$\times$ the average graph summary length to match information access at the prompt level.

\subsection{Models and Metrics}

For IL-TUR we evaluate on two architecturally distinct frontier models: Grok-4.1 and Claude Sonnet 4.5. Since Grok also serves as the extraction model, Sonnet provides an independent cross-model control. For the ECHR set we use GPT-4.1 mini \cite{openai2025gpt41}---which is both extractor and a non-frontier predictor---and GPT-5.4 \cite{openai2026gpt54} as a frontier predictor on the same graphs. Using a non-frontier extractor stress-tests whether the ontology constraints can compensate for weaker upstream extraction. We measure accuracy, macro-F1, calibration (ECE, Brier), and use McNemar's test with bootstrap 95\% CIs. For retrieval, we report nDCG@10, MAP, and P@10 with paired $t$-tests. Our IL-TUR evaluation across 10,068 model runs produced zero parse or API errors.
...
\subsection{Facts-Only Control and Fairness Audits}

To test whether the pipeline's value derives from structuring judicial reasoning or merely from reorganizing factual content, we extracted graphs from only the facts section of 50 cases: facts-only graph (62\%) matches raw facts (64\%) and is far below full graph (90\%), confirming the signal comes from the court's analytical sections rather than the facts. A separate 30-case leakage audit measured comparable outcome-signal density in raw vs.\ graph (0.233 vs.\ 0.225 per 1K chars), and the consistent ordering full graph $>$ raw $>$ structured nodes rules out memorization---no memorization hypothesis explains why removing edges would degrade an extracted graph below raw text. Together these controls attribute the gap to structural organization rather than information advantage.
...
\subsection{Cross-Jurisdictional Evaluation}
\label{sec:echr}

\begin{table}[t]
\small
\centering
\caption{Cross-jurisdictional ECHR evaluation (297 Turkey-respondent HUDOC cases, 88\% violation). Extraction: GPT-4.1 mini. Blinding includes the Indian-SC outcome regex plus ECHR-specific strips for Registry headnote bullets, voting splits, ``violation of Art.~N'' references, and Grand Chamber narrations of prior Chamber decisions (identified via a leak audit; see \S\ref{sec:limitations}). Graph improves accuracy, macro-F1, and Brier at both predictor scales; the GPT-5.4 accuracy gap is significant by bootstrap 95\% CI ($[+.003,+.064]$, excludes 0) and marginal by McNemar ($\chi^2{=}3.68$).}
\label{tab:echr}
\begin{tabular}{llcccc}
\toprule
Predictor & Input & Acc. & Macro-F1 & ECE$\downarrow$ & Brier$\downarrow$ \\
\midrule
\multirow{2}{*}{GPT-4.1 mini} & Raw   & .886 & .522 & \textbf{.067} & .103 \\
                              & Graph & \textbf{.913} & \textbf{.744} & .069 & \textbf{.081} \\
\midrule
\multirow{2}{*}{GPT-5.4}      & Raw   & .909 & .662 & .028 & .077 \\
                              & Graph & \textbf{.943} & \textbf{.845} & \textbf{.026} & \textbf{.055} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:echr} shows that on ECHR, graph representations beat raw text on accuracy, macro-F1, and Brier at both predictor scales, and on ECE for GPT-5.4 (ECE is essentially tied on GPT-4.1 mini). Macro-F1 is the appropriate primary metric given the 88\% class imbalance: the +18.3pp graph advantage on GPT-5.4 (and +22.2pp on GPT-4.1 mini) is driven by minority-class recall, where graphs correctly classify 62.9\% of no-violation cases versus 22.9\% for raw (+40.0pp on GPT-5.4; +37.1pp on GPT-4.1 mini), while both methods correctly identify $>$98\% of violations. The GPT-5.4 accuracy gap of +3.4pp does not reach McNemar significance ($\chi^2{=}3.68$, just below the 3.84 threshold for $p{<}0.05$), but the bootstrap 95\% CI excludes zero ($[+.003,+.064]$), and class-imbalance-compressed headroom (88\% majority baseline) limits achievable accuracy gains.

\paragraph{Capability scaling.}
The graph macro-F1 advantage is large at both predictor scales (+22.2pp on GPT-4.1 mini, +18.3pp on GPT-5.4). No-violation recall rises with predictor capability (+37.1pp $\rightarrow$ +40.0pp), and absolute graph macro-F1 climbs from .744 to .845, consistent with the \S\ref{sec:results} finding that the graph's value lies in relational structure requiring active inference: stronger reasoners more effectively exploit the conjunctive/disjunctive justification structure that distinguishes a no-violation finding (e.g., margin-of-appreciation deference, prescribed-by-law satisfied, legitimate aim served) from a violation finding.

\paragraph{Extraction robustness.}
Graphs were extracted by GPT-4.1 mini, a non-frontier model, with 80\% gold+silver quality tier (vs.\ 90\% gold on IL-TUR with Grok-4.1). Despite this lower extraction quality, the prediction advantage is large at both predictor scales, indicating that ontology-provided edge whitelists and type-level validation compensate for weaker extraction. This evidence---together with the calibration improvement replicating across both jurisdictions---supports the claim that the pipeline's benefits derive from the ontology-driven structural constraints, not from frontier-model extraction.

\paragraph{Raw blinding robustness.}
A leak audit identified ECHR-specific outcome-revealing patterns (Registry headnote bullet chains, Grand Chamber narrations of prior Chamber rulings, voting splits, admissibility narrations) that the Indian-SC regex missed and that were 2$\times$ more prevalent in no-violation cases (54\% vs.\ 28\%); ECHR-specific strips were added before this evaluation. The graph's structural blinding (withholding outcome fields by schema rather than regex) is robust to this failure mode by construction.
...
\paragraph{What does and does not generalize across jurisdictions.}
The ECHR evaluation (\S\ref{sec:echr}) shows the pipeline ports beyond Indian law using a separately compiled 55-concept ontology built under the same schema, with macro-F1 gains of +18--22pp driven by +37--40pp minority-class recall on both OpenAI predictors. Accuracy gains are real but compressed by class imbalance: at 88\% violation, the +40pp minority win yields only +3.4pp aggregate accuracy on GPT-5.4 (bootstrap 95\% CI excludes 0; McNemar marginal at $\chi^2{=}3.68$); macro-F1 is the fairer summary in imbalanced settings. Calibration improvement also replicates (ECE: .028$\rightarrow$.026 on GPT-5.4; Brier favors graph at both scales).
...
\textbf{Proprietary models.} Our IL-TUR results use proprietary frontier models (Grok-4.1 for extraction, Grok and Sonnet for evaluation). The ECHR evaluation (\S\ref{sec:echr}) partially addresses this concern by extracting with GPT-4.1 mini (non-frontier) and reporting predictions on both that model and frontier GPT-5.4, demonstrating the pipeline's benefits persist with weaker extractors. Fully open-weights reproducibility---e.g., extraction with a Llama- or Qwen-class model---remains future work.
...
The approach generalizes beyond legal NLP to long documents over structured diagnostic ontologies (clinical, regulatory).
...
OpenAI. 2025. GPT-4.1 mini [Large language model, API model ID: \texttt{gpt-4.1-mini}]. Accessed via the OpenAI Chat Completions API.

\bibitem[OpenAI(2026)]{openai2026gpt54}
OpenAI. 2026. GPT-5.4 [Large language model, API model ID: \texttt{gpt-5.4}]. Accessed via the OpenAI Chat Completions API.
